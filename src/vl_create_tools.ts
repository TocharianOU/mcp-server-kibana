...
export function registerVLCreateTools(server: ServerBase, kibanaClient: KibanaClient) {
  ...
  server.tool(
    ...
    z.object({
      ...
      id: z.string().optional().describe("OPTIONAL: Specific ID for the saved object. If not provided, Kibana will auto-generate a unique ID. Use this when you need a predictable ID or when recreating objects. RETRY SAFETY: if this call is retried after a timeout or dropped response without an id, Kibana creates a SECOND object with a new auto-generated id -- there is no way to detect that after the fact. Kibana's own PUT-with-id endpoint (used when id is set) already rejects a duplicate create with a 409 unless overwrite is also true, so passing a stable, caller-chosen id here is what makes a retry safe. Do not derive it from the object's attributes alone (e.g. title) -- two different legitimate objects can share the same title, and colliding them would silently discard one."),
